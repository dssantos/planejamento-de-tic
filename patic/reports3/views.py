...
from patic.reports3.models import file_acoes_to_df
from patic.reports3.forms import ReportsForm
...
def report3(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            docfile = request.FILES['planilha_de_excepcionalidade']
            buffer = io.BytesIO()
            buffer.name = 'report3'
            ...
            orgao = filename.replace('  ', ' ').replace('Excepcionalidade PA ', '').replace(' (ultimo)', '')[:-5]
            ano_value = filename.replace('  ', ' ').replace('Excepcionalidade PA ', '').replace(' (ultimo)', '')[-4:]
            ...
            for index, row in df.iterrows():
                ...
                nomerelatorio = Paragraph("<bold><font size=20 color='white'>Relatório de Excepcionalidade</font></bold>", style)
            ...
            buffer.seek(0)
            return FileResponse(buffer, as_attachment=True, filename='Relatório de Excepcionalidade {} {}.pdf'.format(orgao, ano_value))

    else:
        form = ReportsForm()
        return render(request, 'report3/form_report3.html', {'form': form})